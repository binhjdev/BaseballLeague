...
abstract class BaseballDao {
    ...
    abstract fun getGamesForDate(dateString: String): LiveData<List<ScheduleGame>>

    @Query("SELECT * FROM games WHERE gameId = :gameId")
    abstract fun getGameByGameId(gameId: String): ScheduleGame?

    @Insert
    abstract suspend fun insertGame(game: ScheduleGame)

    @Update
    abstract suspend fun updateGame(game: ScheduleGame)

    @Transaction
    open suspend fun insertOrUpdateGames(games: List<ScheduleGame>) {
        games.forEach { game ->
    ...
    abstract suspend fun getPlayerById(playerId: String): Player?
    ...
    abstract suspend fun insertPlayerListItems(
            playerListItem: List<PlayerListItem>
    )
